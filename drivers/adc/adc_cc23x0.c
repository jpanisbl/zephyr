...
struct adc_cc23x0_data {
	...
	uint32_t res;
	uint32_t adj_gain[ADC_CC23X0_MEM_COUNT];
	uint32_t ch_sel[ADC_CC23X0_MEM_COUNT];
	uint32_t mem_index[ADC_CC23X0_CH_COUNT];
...
static void adc_cc23x0_isr(const struct device *dev)
{
	...
	for (i = 0 ; i < ADC_CC23X0_MEM_COUNT ; i++) {
				  ...
				  struct k_poll_signal *sig)
{
	struct adc_cc23x0_data *data = dev->data;
	uint32_t bitmask;
	uint32_t ch_start = ADC_CC23X0_CH_UNDEF;
	uint32_t ch = ADC_CC23X0_CH_UNDEF;
	size_t exp_size;
	int ret;
	int i;
	...
	if (data->ch_count == 1) {
		...
		LOG_ERR("Too many channels in the sequence, max %d", ADC_CC23X0_MEM_COUNT);
		return -EINVAL;
	}
...
static int adc_cc23x0_calc_clk_cfg(uint32_t acq_time_ns, uint32_t *clk_div, uint16_t *clk_cycles)
{
	...
	*clk_cycles = 0;
	...
	ARRAY_FOR_EACH(clk_dividers, i) {
		...
		clock_period_ns = 1.0E9 * divider / base_clock;
		...
		delta_res_ns = samp_duration_ns - acq_time_ns;

		/* Check if this configuration is valid and optimal */
		if (cycles <= min_cycles && delta_res_ns <= min_delta_res) {
			min_delta_res = delta_res_ns;
			min_cycles = cycles;
			*clk_div = divider;
			*clk_cycles = cycles;
				    ...
				    const struct adc_channel_cfg *channel_cfg)
{
	...
	if (data->curr_mem_index > ADC_CC23X0_MEM_MAX) {
		LOG_ERR("%d channels are already configured", ADC_CC23X0_MEM_COUNT);
		return -ENOMEM;
	}

	if (ch > ADC_CC23X0_CH_MAX) {
		LOG_ERR("Channel %d is not supported, max %d", ch, ADC_CC23X0_CH_MAX);
		return -EINVAL;
	}
